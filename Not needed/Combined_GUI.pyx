...
windows = ["self.AdminLoginGUI()","self.DebugGUI()","self.AccountGUI()","self.CAccountGUI()","#self.StartGUI.start(self)","self.SignUpGUI.start(self)","self.LoginGUI.start(self)","self.ManagerGUI.start(self)"]
...
class GUI:
    ...
    class ManagerGUI:
        ...
        def editDevice():
            D = []
            print(app.getTextArea("editPurpose"))
            for i in range(1,len(labels)):
                if i == 6:
